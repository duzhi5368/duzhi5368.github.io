 ...
 define(['text!config.json', 'pixi', 'algorithm/binpacker', 'jquery-ui'], function(config, PIXI, BinPacker) {
	...
	var TexturePacker = Class.extend({
		...
		pack: function() {
			...
			// log skipped frames
			var log = (skippedFrames.length ? skippedFrames.length + " 个图片未能打包.<br /><br />" + skippedFrames.join("<br />") : "");
		...
		btn_new: function() {
			if(!this.changed || confirm("Discard changes and create a new spritesheet?")) {
				...
				window.location.href = "./FKTexturePackerIndex.html";
			}
		},
		...
		btn_publish: function() {
			...
			output += "\n},\n";
			output += "\"meta\": {\n";
			output += "\t\"app\": \"http://www.FreeKnightGame.com/FKTexturePacker/\",\n";
			output += "\t\"version\": \"1.0\",\n";
